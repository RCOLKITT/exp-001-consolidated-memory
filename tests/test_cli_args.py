...
def test_cli_accepts_workflow_flags(mod, extra, tmp_path):
    ...
    r = subprocess.run([sys.executable, "-m", mod, *extra, *(PROVIDER if mod in ("phase3.learn", "phase4.evaluate", "phase0.run_control") else []), *(SEAM if mod in ("phase3.learn", "phase4.evaluate") else []), "--help"],
